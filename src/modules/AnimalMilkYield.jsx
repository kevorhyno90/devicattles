...
const SAMPLE = [
  { id: 'MILK-001', animalId: 'A-001', date: '2025-06-02', liters: 18.5 }
]
...
export default function AnimalMilkYield({ animals }){
  ...
  function add(){
    if(!animalId || !liters) return
    const id = 'MILK-' + Math.floor(1000 + Math.random()*9000)
    setItems([...items, { id, animalId, date: new Date().toISOString().slice(0,10), liters: Number(liters) }])
    setLiters('')
  }
  ...
  return (
    <section>
      ...
      <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:12}}>
        ...
        <input placeholder="Liters" value={liters} onChange={e=>setLiters(e.target.value)} style={{width:120}} />
      ...
      <ul style={{listStyle:'none',padding:0}}>
        {items.map(it => (
          <li key={it.id} style={{padding:8,borderBottom:'1px solid #eee',display:'flex',justifyContent:'space-between'}}>
            <div>
              <div style={{fontWeight:600}}>{it.liters} L <small style={{color:'#666'}}>({it.id})</small></div>

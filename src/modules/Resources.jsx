          ...
          <li key={r.id} style={{padding:10, borderBottom:'1px solid #eee'}}>
            {inlineEditId === r.id ? (
              ...
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                <div style={{flex:1}}>
                  <div style={{fontWeight:600}}>{r.name} <small style={{color: 'var(--text-secondary)'}}>({r.id})</small></div>
                  <div style={{fontSize:13,color: 'var(--text-secondary)'}}>{r.qty} units • {r.condition}</div>
                </div>
